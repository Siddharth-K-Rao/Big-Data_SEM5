...
def matchupdate(matchdict,date,label):
    ...
    try:
        with open(match_events,"r") as json_file:
            fulldata=json.load(json_file)
            if datatuple in fulldata:
                return
    except:
        fulldata={}
    fulldata={}
...
#print("DONe")
ssc=StreamingContext(sc,2) #5s for time difference between two batches - design decision
...
def matchupdate(matchdict,date,label):
    ...
    df=sqlcontext.read.option('multiLine',True).json(match_events)
    try:
        k=df[datatuple]
        print("YES")        
    except:
